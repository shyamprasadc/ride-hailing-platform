...
const validateRideRequest = (request: CreateRideRequest): Result<true> => {
  if (!request.riderId) {
    throw new UnprocessableEntityError('Rider ID is required');
  }

  if (!request.pickup.lat || !request.pickup.lng) {
    throw new UnprocessableEntityError('Valid pickup location is required');
  }

  if (!request.dropoff.lat || !request.dropoff.lng) {
    throw new UnprocessableEntityError('Valid dropoff location is required');
  }

  if (request.pickup.lat < -90 || request.pickup.lat > 90) {
    throw new UnprocessableEntityError('Invalid pickup latitude');
  }

  if (request.pickup.lng < -180 || request.pickup.lng > 180) {
    throw new UnprocessableEntityError('Invalid pickup longitude');
  }

  if (request.dropoff.lat < -90 || request.dropoff.lat > 90) {
    throw new UnprocessableEntityError('Invalid dropoff latitude');
  }

  if (request.dropoff.lng < -180 || request.dropoff.lng > 180) {
    throw new UnprocessableEntityError('Invalid dropoff longitude');
  }

  if (!Object.values(RideType).includes(request.rideType)) {
    throw new UnprocessableEntityError('Invalid ride type');
  }
  ...
  while (attempts < options.maxAttempts && !matched) {
    ...
    for (const driver of sortedDrivers) {
      ...
      if (matchResult.success) {
        ...
        Logger.info(`Failed to match with driver ${driver.driverId}:`, matchResult.error?.message);
      }
    }

...
const validateLocation = (
  ...
  if (location.longitude < -180 || location.longitude > 180) {
    return { success: false, error: new Error('Invalid longitude') };
  }

  if (location.speed !== undefined && location.speed < 0) {
    return { success: false, error: new Error('Speed cannot be negative') };
  }
  ...
  try {
    ...
    if (!driver) {
      ...
      if (!dbDriver) {
        return {
          success: false,
          error: new NotFoundError('Driver not found'),
        };
      }

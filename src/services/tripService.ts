...
export const startTrip = async (request: StartTripRequest): Promise<Result<TripResponse>> => {
  return await prisma.$transaction(async (tx) => {
    ...
    if (trip.status !== TripStatus.PENDING) {
      throw new BadRequestError('Trip already started or completed');
    }
...
export const endTrip = async (request: EndTripRequest): Promise<Result<TripResponse>> => {
  return await prisma.$transaction(async (tx) => {
    ...
    if (trip.status !== TripStatus.STARTED) {
      throw new BadRequestError('Trip not in progress');
    }

    if (!trip.startTime) {
      throw new BadRequestError('Trip start time not found');
    }

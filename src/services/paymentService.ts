...
export const processPayment = async (
  ...
  return await prisma.$transaction(async (tx) => {
    ...
    if (trip.status !== TripStatus.COMPLETED) {
      throw new UnprocessableEntityError('Trip not completed');
    }
...
export const retryPayment = async (paymentId: string): Promise<Result<PaymentResponse>> => {
  return await prisma.$transaction(async (tx) => {
    ...
    if (payment.status === PaymentStatus.COMPLETED) {
      throw new UnprocessableEntityError('Payment already completed');
    }

    if (payment.attempts >= payment.maxAttempts) {
      throw new UnprocessableEntityError('Maximum retry attempts reached');
    }
...
export const processRefund = async (
  ...
  return await prisma.$transaction(async (tx) => {
    ...
    if (payment.status !== PaymentStatus.COMPLETED) {
      throw new UnprocessableEntityError('Only completed payments can be refunded');
    }

    if (amount > payment.amount) {
      throw new UnprocessableEntityError('Refund amount exceeds payment amount');
    }
